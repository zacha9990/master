...
// slimmenu initialisation
    $('#navigation').slimmenu({
        ...
        easingEffect: null,
        indentChildren: true,
        expandIcon: '<i class="ti-angle-right"></i>',
        collapseIcon: '<i class="ti-angle-down"></i>'
    });
    var collapse_button = $('.collapse-button');
    $(document).click(function (e) {
        var container = $("#navigation,.menu-collapser");
        if (!container.is(e.target) // if the target of the click isn't the container...
            && container.has(e.target).length === 0 && $(window).width() <= 992) // ... nor a descendant of the container
        {
